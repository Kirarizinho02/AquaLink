...
import { FaInfo, FaStore, FaUser } from "react-icons/fa6";
...
const navigationLinks = [
  ...
  { href: "/shop", label: "Loja", icon: FaStore },
];

const HeaderShop = () => {
  const { user, logout } = useAuthContext();

  const [open, setOpen] = useState(false); // dropdown perfil
  const [cartOpen, setCartOpen] = useState(false); // sheet do carrinho
    ...
    <header className="px-4 md:px-6 py-5 margin mx-6 border-b max-w-[1880px] w-full">
      <div className="flex h-16 items-center justify-between gap-4">
        ...
        <div className="flex flex-1 items-center justify-end gap-4">
          {/* Carrinho (substitui o ThemeSwitcher na barra) */}
          <Button
            variant="ghost"
            size="icon"
      ...
      <Sheet open={cartOpen} onOpenChange={setCartOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md">
          <SheetHeader>
            <SheetTitle className="font-semibold text-2xl">Carrinho</SheetTitle>
            <SheetDescription>Seus itens aparecerão aqui.</SheetDescription>
          </SheetHeader>
